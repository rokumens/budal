$(document).ready(function() {
	...
	$("#xin-form").submit(function(e){
		...
		fd.append("is_ajax", 1);
		fd.append("add_type", 'add_punishment');
		fd.append("form", action);
		e.preventDefault();
		$('.btn').prop('disabled', true); // luffy 8 January 2020 04:04 pm
		$.ajax({
			url: e.target.action,
			type: "POST",
			data:  fd,
	...
	$('.edit-modal-data').on('show.bs.modal', function (event) {
		...
		var punishment_id = button.data('punishment_id');
		var modal = $(this);
		$.ajax({
			url : base_url+"/read/",
			type: "GET",
			data: 'jd=1&is_ajax=1&mode=modal&data=punishment_update&punishment_id='+punishment_id,
	...
	$.ajax({
		url: base_url+'/read/',
		type: "GET",
		data: 'jd=1&is_ajax=4&mode=modal&data=view_punishment_task&type=view_punishment_task&punishment_id='+punishment_id,
		success: function (response) {
			if(response) {
