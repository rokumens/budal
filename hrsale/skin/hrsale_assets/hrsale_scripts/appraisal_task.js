$(document).ready(function() {
	...
	$('.minMonthlyReqA').bind('change', function () {	//use keyup mouseup kalo inbox
			var minMonthlyReqA = this.value;
			let result = Math.round(minMonthlyReqA/30);
	    $('.minDailyReqA').val(result);
	});
	$('.minMonthlyReqB').bind('change', function () {	//use keyup mouseup kalo inbox
			var minMonthlyReqB = this.value;
			let result = Math.round(minMonthlyReqB/30);
	    $('.minDailyReqB').val(result);
	});
	...
	$('.edit-modal-data').on('show.bs.modal', function (event) {
		...
		$.ajax({
			url : base_url+"/read/",
			type: "GET",
			data: 'jd=1&is_ajax=1&mode=modal&data=job_task_update&appraisal_task_id='+appraisal_task_id,
