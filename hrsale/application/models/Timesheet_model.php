...
class Timesheet_model extends CI_Model{
  ...
  public function __construct(){
	...
	public function attendance_first_out_check($employee_id,$attendance_date) {
		...
		$sql = 'SELECT * FROM xin_attendance_time WHERE employee_id = ? and attendance_date = ? ORDER BY attendance_date desc limit 1';
		$binds = array($employee_id,$attendance_date);
		$query = $this->db->query($sql, $binds);
	...
	public function get_leaves() {
		$sql = 'SELECT * FROM xin_leave_applications ORDER BY leave_id DESC';
	...
	public function count_total_leaves($leave_type_id,$employee_id) {
		...
		$binds = array($employee_id,$leave_type_id,1);
		$query = $this->db->query($sql, $binds);
	...
	public function getLeaveById($id) {
		$sql = 'SELECT * FROM xin_leave_applications WHERE leave_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds);
		if ($query->num_rows() > 0) {
			return $query;
	...
	public function check_user_attendance_clockout() {
		$today_date = date('Y-m-d');
		$session = $this->session->userdata('username');
		$sql = 'SELECT * FROM xin_attendance_time where `employee_id` = ? and `attendance_date` = ? and clock_out = ? ORDER BY attendance_date desc limit 1';
		$binds = array($session['user_id'],$today_date,'');
		$query = $this->db->query($sql, $binds);
	...
	public function get_last_user_attendance() {
		...
		$session = $this->session->userdata('username');
		$sql = 'SELECT * FROM xin_attendance_time where `employee_id` = ? ORDER BY attendance_date desc limit 1';
	...
	public function attendance_time_checks($id) {
		...
		$session = $this->session->userdata('username');
		$sql = 'SELECT * FROM xin_attendance_time where `employee_id` = ? and clock_out = ? ORDER BY attendance_date desc limit 1';
		$binds = array($id,'');
		$query = $this->db->query($sql, $binds);
