...
class Xin_model extends CI_Model {
	...
	{
	  $query = $this->db->query("SELECT * from xin_leave_applications where employee_id = '".$user_id."' order by leave_id desc limit 5");
  	  return $query->result();
	}
	...
	public function currency_sign($number) {
		...
		if($system_setting[0]->currency_position=='Prefix'){
			$sign_value = $sc_show.' '.number_format($number,0,'','.');
		} else {
			$sign_value = number_format($number,0,'','.').' '.$sc_show;
		}
		return $sign_value;
	}
	...
	{
	  $query = $this->db->query("SELECT * from xin_employees WHERE is_active = 1 AND deleted_at IS NULL AND fingerprint_location != 0");
  	  return $query;
	}
  ...
  // luffy 11 Dec 2019 - 02:53 pm
	// get current day late
	public function current_month_day_late($current_month) {
		$session = $this->session->userdata('username');
		$sql = "SELECT employee_id,attendance_date FROM xin_attendance_time WHERE attendance_date = ? AND late <> '' GROUP BY employee_id";
		$binds = array($current_month);
		$query = $this->db->query($sql, $binds);
		return $query->num_rows();
	}
	...
	{
	 	$sql = 'SELECT * FROM xin_employees WHERE is_active = ? AND deleted_at IS NULL AND fingerprint_location != 0';
		$binds = array(1);
		$query = $this->db->query($sql, $binds);
  	  	return $query->result();
	}
	...
	{
		$sql = 'SELECT * FROM xin_employees WHERE gender = ? AND is_active = 1 AND deleted_at IS NULL AND fingerprint_location != 0';
		$binds = array('Male');
		$query = $this->db->query($sql, $binds);
  ...
  # luffy 31 Dec 2019 10:50 am
	// get all approver / approval
	public function get_approver_list() {
	 	return  $query = $this->db->query("SELECT * from luffy_approver");
	}
  # luffy 31 Dec 2019 07:32 pm
	// get approver by employee user id
  public function getApproverByUserId($userId) {
    $sql = 'SELECT * FROM luffy_approver WHERE approver_id=?';
    $binds = array($userId);
    $query = $this->db->query($sql, $binds);
    if($query->num_rows() > 0)
      return $query->row();
  ...
  # luffy 29 Dec 2019 08:03 pm | adding created by and at
  ...
  # luffy 29 Dec 2019 08:03 pm | adding created by and at
	public function add_approver($data){
    $session=$this->session->userdata('username');
    $userId=$session['user_id'];
    $this->db->set('created_at',date('Y-m-d H:i:s'));
    $this->db->set('created_by',$userId);
		$this->db->insert('luffy_approver', $data);
		if ($this->db->affected_rows() > 0)
			return true;
		else return false;
	}
  ...
  // luffy
	// Function to add record in table
	public function add_approval_status($data){
		$this->db->insert('xin_approval_status', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
  ...
  // 31 December 2019 12:28 pm
	// Delete approver from approver list in the Warning Type section.
	public function delete_approver_record($id){
		$this->db->where('approver_id', $id);
		$this->db->delete('luffy_approver');
	}
	// Function to Delete selected record from table
	...
	public function get_last_payment_history() {
	  $query = $this->db->query("SELECT * from xin_salary_payslips order by payslip_id desc limit 5");
  	  return $query->result();
	}
	...
	public function employees_upcoming_birthday() {
		...
		$query = $this->db->query("SELECT `user_id`, `first_name`, `last_name`, `date_of_birth`, employee_id, username,
    DATE_ADD(
        date_of_birth,
...
FROM `xin_employees`
WHERE
    `date_of_birth` IS NOT NULL AND is_active=1 AND fingerprint_location!=0 AND deleted_at IS NULL
HAVING
...
ORDER BY `next_birthday`");
  ...
  public function company_license_expiry() {
    $sql = "SELECT document_id, expiry_date, license_name, license_number, notification_date_1, notification_date_2, is_sent_notification_1 AS notif1, is_sent_notification_2 AS notif2,
    DATE_ADD(
        expiry_date,
    ...
    WHERE
      ...
      (DATE(notification_date_1) = CURDATE()) OR  -- showing first notification only on that day. // luffy
      (DATE(notification_date_2) <= CURDATE())    -- showing second notification by interval. // luffy
    HAVING expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 1 WEEK) -- keep showing second notif till expiry date updated.
    ORDER BY expiry_date asc";  // the closest expired date first to show.
		$query = $this->db->query($sql);
    return $query->result();
	}
  // luffy 21 nov 2019 08:09 pm
  public function employee_license_expiry() {
    $sql = "SELECT
      document.employee_id,document.document_id, document.date_of_expiry, document.title, document.is_sent_notification_1 AS notif1, document.is_sent_notification_2 AS notif2, document.notification_date_1, document.notification_date_2,
      DATE_ADD(
          document.date_of_expiry,
          INTERVAL IF(DAYOFYEAR(document.date_of_expiry) >= DAYOFYEAR(CURDATE()),
              YEAR(CURDATE())-YEAR(document.date_of_expiry),
              YEAR(CURDATE())-YEAR(document.date_of_expiry)+1
          ) YEAR
      ) AS expiry_date,
      employee.employee_id AS employeeNIK, employee.first_name, employee.last_name, employee.username,
      doctype.document_type,
      immigration.document_number
    FROM xin_employee_documents AS document
    LEFT JOIN xin_employees AS employee ON employee.user_id = document.employee_id
    LEFT JOIN xin_document_type AS doctype ON doctype.document_type_id = document.document_type_id
    LEFT JOIN xin_employee_immigration AS immigration ON immigration.employee_id = document.employee_id AND doctype.document_type_id = immigration.document_type_id
    WHERE
      (document.date_of_expiry IS NOT NULL) AND
      (DATE(document.notification_date_1) = CURDATE()) OR  -- showing first notification only on that day. // luffy
      (DATE(document.notification_date_2) <= CURDATE())    -- showing second notification by interval. // luffy
    HAVING document.date_of_expiry BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 1 WEEK) -- keep showing second notif till expiry date updated.
    ORDER BY document.date_of_expiry asc";  // the closest expired date first to show.
		$query = $this->db->query($sql);
  ...
  // luffy start by assigned to (untuk di notification header)
  public function get_my_assigned_ticket($employeeId) {
    $sql = 'SELECT * FROM xin_support_tickets WHERE assigned_to = ?';
