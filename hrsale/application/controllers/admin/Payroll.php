...
class Payroll extends MY_Controller {
  ...
  public function advance_salary_report_list() {
    ...
    foreach($advance_salary->result() as $r) {
			...
			if($r->advance_amount == $r->total_paid)
			 ...
			 else $all_paid = '<span class="badge badge-warning">'.$this->lang->line('xin_remaining').'</span>';
			//total paid
    ...
    foreach($payslip->result() as $r) {
		  // user full name
			...
			}elseif($r->monthly_grade_id !='' || $r->monthly_grade_id !=0) {
    ...
    foreach($payslip->result() as $r) {
		  // user full name
			...
			$location = $r->fingerprint_location;
			...
			$add_salary = $basic_salary + $allowance_amount ;
			// add amount
			...
			// Adjustment (-)
			$arrAdjustmentMinus = $this->Employees_model->all_adjustment_minus($r->user_id);
			$adjustmentMinusAmount = 0;
			if(count($arrAdjustmentMinus->result()) > 0) {
				foreach($arrAdjustmentMinus->result() as $singAdjustmentMinus){
					$adjustmentMinusAmount += $singAdjustmentMinus->adjustment_minus_amount;
				}
			} else {
				$adjustmentMinusAmount = 0;
			}
			$net_salary = $net_salary_default - $adjustmentMinusAmount;
			...
			if($payment_check->num_rows() > 0){
				...
				$mpay = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_payroll_view_payslip').'"><a href="'.site_url().'admin/payroll/payslip/id/'.$make_payment[0]->payslip_id.'">
				<button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-arrow-circle-right"></span></button></a></span>
				<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_download').'"><a href="'.site_url().'admin/payroll/pdf_create/p/'.$make_payment[0]->payslip_id.'">
				<button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light">
				<span class="fa fa-download"></span></button></a></span>';
			} else {
    ...
    // get addd by > template
		...
		if(!empty($session))
			$this->load->view('admin/payroll/dialog_make_payment', $data); //pay_salary()
		else redirect('admin/');
	}
	...
	public function add_pay_monthly() {
		if($this->input->post('add_type')=='add_monthly_payment') {
			...
			$data = array(
				...
				'total_adjustment_minus' => $this->input->post('total_adjustment_minus'),
			...
			if ($result) {
				...
				$arrAdjustmentMinus = $this->Employees_model->all_adjustment_minus($this->input->post('emp_id'));
				if(count($arrAdjustmentMinus->result()) > 0) {
					foreach($arrAdjustmentMinus->result() as $singAdjustmentMinus){
						$adjustment_minus_data = array(
							'payslip_id' => $result,
							'employee_id' => $this->input->post('emp_id'),
							'salary_month' => $this->input->post('pay_date'),
							'adjustment_minus_title' => $singAdjustmentMinus->adjustment_minus_title,
							'adjustment_minus_amount' => $singAdjustmentMinus->adjustment_minus_amount,
							'created_at' => date('Y-m-d H:i:s')
						);
						$_adjustment_minus_data = $this->Payroll_model->add_salary_payslip_adjustment_minus($adjustment_minus_data);
					}
				}
  ...
  public function payslip() {
		...
		$data = array(
			...
			'total_adjustment_minus' => $result[0]->total_adjustment_minus,
  ...
  public function pdf_create() {
	  ...
	  $author=$creator->employee_id.' - '.$creator->username;
		...
		//luffy 28 nov 2019 - 01:55 pm
		// $tbl = '
		// <div style="border:1px solid #ccc; padding:2px; border-bottom: 2px solid #000;">
    ...
    // $tbl .= "
		...
		// 		<tr>
		// 			<td align="center">
		// 				<img src="'.$clogo.'" height="40" width="auto" class="logoApgCenter" />
		// 			</td>
		// 		</tr>
		// 	</table>
		// 	<br />";
		$tbl .= '
			<div>
				<table cellpadding="10" cellspacing="0" border="0" class="garisTable">
					<tr>
						<td border="0" style="border:2px solid #444; padding:2px; border-bottom:2px solid #444;">
							<table>
								<tr>
									<td>&nbsp;</td>
								</tr>
								...
								<tr>
									<td align="center">
										<h2 style="font-weight:normal;">'.$company_name.' CORP LTD</h2>
									</td>
								</tr>
								<tr>
									<td>&nbsp;</td>
    ...
    $overtime = $this->Employees_model->set_employee_overtime_payslip($payment[0]->payslip_id);
		...
		$tbl .= '
			<div>
				<table cellpadding="5" border="0">
					<tr>
						<td colspan="3">
							<nobr><i><strong>Adjustment (+)</strong></i></nobr>
						</td>
						<td colspan="3">
							<nobr><i><strong>Adjustment (-)</strong></i></nobr>
						</td>
							...
							<table>
								<tr>
									<td width="160" height="25">
										<nobr>Gaji All In</nobr>
									</td>
									<td width="25" height="25"><nobr> = </nobr></td>
									<td align="right" width="105" height="25">
										'.$this->Xin_model->currency_sign($bs).'
									</td>
								</tr>';
							$totalAdjustmentPlus = 0;
							$arrAdjustmentPlus = $this->Employees_model->set_employee_allowances_payslip($payment[0]->payslip_id);
							if(count($arrAdjustmentPlus->result()) > 0) {
								foreach($arrAdjustmentPlus->result() as $singAdjustmentPlus){
									$totalAdjustmentPlus += $singAdjustmentPlus->allowance_amount;
					$tbl .='<tr>
										<td width="160" height="25">
											<nobr>'.$singAdjustmentPlus->allowance_title.'</nobr>
										</td>
										<td width="25" height="25"><nobr> = </nobr></td>
										<td align="right" width="105" height="25">'.$this->Xin_model->currency_sign($singAdjustmentPlus->allowance_amount).'</td>
									</tr>';
									}
								}
			$tbl .='</table>
						...
						<td width="319">
							<table>';
								...
								$arrAdjustmentMinus = $this->Employees_model->getAdjustmentMinusByPayslipid($payment[0]->payslip_id);
								if(count($arrAdjustmentMinus->result()) > 0) {
									foreach($arrAdjustmentMinus->result() as $singAdjustmentMinus){
											$totalAdjustmentMinus += $singAdjustmentMinus->adjustment_minus_amount;
						$tbl .='<tr>
											<td width="160" height="25">
												<nobr>'.$singAdjustmentMinus->adjustment_minus_title.'</nobr>
											</td>
											<td width="25" height="25"><nobr> = </nobr></td>
											<td align="right" width="120" height="25">('.$this->Xin_model->currency_sign($singAdjustmentMinus->adjustment_minus_amount).')</td>
										</tr>';
									}
								}
				$tbl .='</table>
					...
					<tr style="border-bottom:1px solid red !important;">
						<td width="160">
							<h4><i>Gross Income</i></h4>
						</td>
						<td width="25"><nobr> = </nobr></td>
						<td align="right" width="134">';
						$grossIncome=$totalAdjustmentPlus+$bs;
						$totalDeduction=$totalAdjustmentMinus;
			$tbl .='<nobr><h4><i>'.$this->Xin_model->currency_sign($grossIncome).'</i>&nbsp;&nbsp;&nbsp;&nbsp;</h4></nobr>
						</td>

						<td width="160" style="font-family:Calibri;font-size:10px;color:#000000;">
							<h4><i>Total Deduction</i></h4>
						</td>
						<td width="25"><nobr> = </nobr></td>
						<td align="right" width="144">';
			$tbl .='<nobr><h4><i>'.$this->Xin_model->currency_sign($totalDeduction).'</i>&nbsp;&nbsp;&nbsp;&nbsp;</h4></nobr>
						</td>
					</tr>
					...
					<tr>
						<td colspan="3"></td>
						<td width="160" style="font-family:Calibri;font-size:10px;color:#000000;">
							<h4><i>Take Home Pay</i></h4>
						</td>
						<td width="25"><nobr> = </nobr></td>
						<td align="right" width="134">';
						$takeHomePay=$grossIncome-$totalDeduction;
			$tbl .='<nobr><h4><i>'.$this->Xin_model->currency_sign($takeHomePay).'</i></h4></nobr>
						</td>
					</tr>
					...
					<tr>
						<td colspan="4">&nbsp;</td>
						<td colspan="2" align="center" width="180">
							Diterima Oleh,
							<br /><br /><br /><br /><br />
							'.$fname.'
						</td>
		...
		// $file=$pdf->Output('payslip_'.$fname.'_'.$pay_month.'.pdf', 'I');
		// $fp = fopen($file, "r") ;
  ...
  public function get_employees() {
    ...
    // get addd by > template
		...
		if(!empty($session))
			$this->load->view('admin/payroll/dialog_make_payment', $data); // pay_hourly()
		else redirect('admin/');
	}
	...
	public function add_template() {
		...
		} elseif($this->input->post('salary_grades')==='') {
    	$Return['error'] = $this->lang->line('xin_error_template_name');
		} elseif($this->input->post('basic_salary')==='') {
	...
	public function add_hourly_rate() {
		...
		if($this->input->post('company')==='') {
   		$Return['error'] = $this->lang->line('xin_error_company');
		}elseif($this->input->post('hourly_grade')==='') {
    	$Return['error'] = $this->lang->line('xin_error_title');
		}elseif($this->input->post('hourly_rate')==='') {
  ...
  public function employee_payslip_list() {
    ...
    foreach($history->result() as $r) {
		 ...
		 $functions = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view').'"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".detail_modal_data" data-employee_id="'. $r->employee_id . '" data-pay_id="'. $r->make_payment_id . '"><i class="fa fa-arrow-circle-right"></i></button></span>';
		  if($r->payment_method==1){
			  ...
			  $p_method = 'PayPal';
		  } elseif($r->payment_method==3) {
			  $p_method = 'Payoneer';
		  } elseif($r->payment_method==4){
			  $p_method = 'Bank Transfer';
		  } elseif($r->payment_method==5) {
			  $p_method = 'Cheque';
		  } else {
			  $p_method = 'Cash';
		  }
	...
	public function add_currency_data() {
		...
		} elseif($this->input->post('to_currency_rate')==='') {
