  ...
  /*Function to set JSON output*/
    ...
    $this->load->model('Xin_model');
	  ...
	  $this->load->model('Location_model');
		# luffy 10 January 2020 01:41 pm
	  $this->load->model('Employees_model');
	...
	public function index() {
		...
		$data['locations'] = $this->Location_model->get_locations()->result();
    ...
    foreach($assets_category->result() as $r) {
			...
			if(in_array('267',$role_resources_ids))  // update
    ...
    foreach($assets->result() as $r) {
			...
			$assetLocationName=$this->Assets_model->locationNameById($r->asset_location)->location_name;
			$data[] = array($combhr,
				$r->name,
				$category,
	...
	public function asset_read(){
		...
		$locations=$this->Location_model->get_locations()->result();
		$locationId=$result[0]->asset_location;
		$assetLocationName=$this->Assets_model->locationNameById($locationId)->location_name;
		$data = array(
			'locations' => $locations,
			...
			# luffy 10 January 2020 01:42 pm
	...
	public function add_asset() {
		...
		} elseif($this->input->post('asset_name')==='') {
		...
		} elseif($this->input->post('assetPrice')==='') {
      	$Return['error'] = 'Price can not be empty.';
		} elseif($this->input->post('asset_location')==='') {
      	$Return['error'] = 'Please choose asset location';
		...
		$data = array(
			...
			'company_id' => $this->input->post('company_id'),
			'price' => $this->input->post('assetPrice'),
	...
	public function update_asset() {
		...
		if($this->input->post('category_id')==='') {
      	...
      	$Return['error'] = $this->lang->line('xin_error_asset_name_field');
		} /*elseif($this->input->post('company_asset_code')==='') {
      	$Return['error'] = $this->lang->line('xin_error_cat_name_field');
		}*/ elseif($this->input->post('company_id')==='') {
      	$Return['error'] = $this->lang->line('error_company_field');
		} elseif($this->input->post('employee_id')==='') {
      	...
      	$Return['error'] = 'Please choose asset location';
		} /*elseif($this->input->post('asset_note')==='') {
      	$Return['error'] = $this->lang->line('xin_error_asset_note_field');
		}*/
		/* Check if file uploaded..*/
		else if($_FILES['asset_image']['size'] == 0) {
