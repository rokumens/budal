...
class Awards extends MY_Controller {
  ...
  public function award_list(){
		...
		else $award = $this->Awards_model->get_awards();
		$data = array();
	...
	public function add_award() {
		if($this->input->post('add_type')=='award') {
	      ...
	      $Return['error'] = $this->lang->line('xin_error_employee_id');
			}elseif($this->input->post('award_type_id')==='') {
				$Return['error'] = $this->lang->line('xin_award_error_award_type');
			}elseif($this->input->post('award_date')==='') {
			...
			}elseif($this->input->post('cash')!='' && $_FILES['award_picture']['size'] == 0) {
