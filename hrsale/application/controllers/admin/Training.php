...
class Training extends MY_Controller {
    ...
    parent::__construct();
		...
		# luffy 11 January 2020 11:34 am
  ...
  public function training_list() {
    ...
    foreach($training->result() as $r) {
			if(in_array('378',$role_resources_ids)) {
				 # luffy 11 January 2020 08:41 pm
				 ...
				 // foreach($aim as $dIds) {
					//  if($session['user_id'] == $dIds) {
							...
							if(in_array('342',$role_resources_ids))  // update
				...
				if($r->employee_id == '') {
					...
					foreach(explode(',',$r->employee_id) as $uid) {
						$user = $this->Xin_model->read_user_info($uid);
						if(!is_null($user)){
							$assigned_name = $user[0]->first_name.' '.$user[0]->last_name.' ('.$user[0]->fingerprint_location.')';
  ...
  public function get_employees() {
	...
	public function read(){
		...
		$data = array(
			...
			# luffy 11 January 2020 11:35 am
	...
	public function add_training() {
		if($this->input->post('add_type')=='training') {
      	...
      	$Return['error'] = $this->lang->line('error_company_field');
			}elseif($this->input->post('training_type')==='') {
      	$Return['error'] = $this->lang->line('xin_error_training_type');
			}elseif($this->input->post('trainer')==='') {
