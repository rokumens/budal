...
class Accounting extends MY_Controller
{
   ...
   /*Function to set JSON output*/
	...
	public function __construct(){
		...
		$this->load->model('API_form_model');
  ...
  // luffy start
	...
	public function financeform(){
		...
		$data['title'] = 'Finance Form | '.$this->Xin_model->site_title();
		$data['breadcrumbs'] = 'Finance Form';
		$data['path_url'] = 'financeform';
		$data['all_companies'] = $this->Xin_model->get_companies();
		$data['all_user_roles'] = $this->Roles_model->all_user_roles();
		$data['all_api_form'] = $this->API_form_model->get_api_form_idname();
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(in_array('116',$role_resources_ids)) {
		 $data['subview'] = $this->load->view("admin/accounting/financeform", $data, TRUE);
	...
	public function financeform_list() {
		$data['title'] = $this->Xin_model->site_title();
		$session = $this->session->userdata('username');
		if(!empty($session)){
			$this->load->view("admin/accounting/financeform", $data);
		} else {
			redirect('admin/');
		}
		...
		$formzz = $this->API_form_model->get_api_form_data($formId);
		$data = array();
		foreach($formzz->result() as $r) {
			$url=$r->url_view_entry;
			$view = "<a href='$url' target='_blank'>view</a> &raquo;";
			$data[] = array(
				$r->form_name,
				'#'.$r->entry_number,
				$this->Xin_model->set_date_format($r->date_created),
				"$ ".number_format($r->harga_usd,0,',',','),
				"Rp. ".number_format($r->harga_idr,0,',','.'),
				$view
			);
	  }
	  $output = array(
		   "draw" => $draw,
			 "recordsTotal" => $formzz->num_rows(),
			 "recordsFiltered" => $formzz->num_rows(),
			 "data" => $data
		);
