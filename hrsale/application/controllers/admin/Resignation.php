...
class Resignation extends MY_Controller {
    ...
    public function resignation_list(){
	    ...
	    foreach($resignation->result() as $r) {
				...
				if(!is_null($employee)){
					...
					$employee_location = $employee[0]->fingerprint_location;
				} else {
					$employee_name = '--';
					$employee_location = 'X';
				}
				...
				if(in_array('214',$role_resources_ids))  //edit
					$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-resignation_id="'. $r->resignation_id . '"><span class="fa fa-pencil"></span></button></span>';
				else $edit = '';
				if(in_array('215',$role_resources_ids))  // delete
					$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->resignation_id . '"><span class="fa fa-trash"></span></button></span>';
				else $delete = '';
				if(in_array('234',$role_resources_ids))  //view
					$view = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-resignation_id="'. $r->resignation_id . '"><span class="fa fa-eye"></span></button></span>';
				else $view = '';
	...
	public function add_resignation() {
		...
		} elseif($this->input->post('resignation_date')==='') {
