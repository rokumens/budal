...
class Designation extends MY_Controller {
    ...
    foreach($designation->result() as $r){
			...
			if(in_array('244',$role_resources_ids)) // update
				$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"  data-toggle="modal" data-target="#edit-modal-data"  data-designation_id="'. $r->designation_id . '"><span class="fa fa-pencil"></span></button></span>';
			else $edit = '';
			if(in_array('245',$role_resources_ids)) // del
				$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->designation_id . '"><span class="fa fa-trash"></span></button></span>';
			else $delete = '';
			$combhr = $edit.$delete;
      $data[] = array(
	   		$combhr,
	 ...
	 public function deleted(){
			...
			if(empty($session))
				redirect('admin/');
			$data['title'] = 'Deleted '.$this->lang->line('xin_designations').' | '.$this->Xin_model->site_title();
			$data['all_departments'] = $this->Department_model->all_departments();
			$data['get_all_companies'] = $this->Xin_model->get_companies();
			//$data['all_designations'] = $this->Designation_model->all_designations();
			$data['breadcrumbs'] = $this->lang->line('xin_designations');
			...
			$data['userRole']=$userRole;
			if(in_array('4',$role_resources_ids)) {
				$data['subview'] = $this->load->view("admin/designation/designation_list_deleted", $data, TRUE);
				$this->load->view('admin/layout/layout_main', $data); //page load
			} else {
				redirect('admin/dashboard');
			}
    }
    public function designation_list_deleted(){
		$session = $this->session->userdata('username');
		$data['title'] = $this->Xin_model->site_title();
		if(!empty($session))
			$this->load->view("admin/designation/designation_list_deleted", $data);
		else redirect('admin/');
		$draw = intval($this->input->get("draw"));
		$start = intval($this->input->get("start"));
		$length = intval($this->input->get("length"));
		$designation = $this->Designation_model->get_designations_deleted();
		$role_resources_ids = $this->Xin_model->user_role_resource();
		$data = array();
    foreach($designation->result() as $r){
			// get department
			...
			if($user[0]->user_role_id==1)
				if(!empty($session))
					$restore='<span data-toggle="tooltip" data-placement="top" title="Restore"><button type="button" class="btn icon-btn btn-xs btn-success waves-effect waves-light restore" data-toggle="modal" data-target=".restore-modal" data-record-id="'. $r->designation_id . '"><span class="fa fa-undo"></span></button></span>';
		  $action=$restore;
      $data[] = array(
	   		$action,
        $r->designation_name,
	...
	public function add_designation(){
		if($this->input->post('add_type')=='designation') {
			...
			}elseif($this->input->post('designation_name')===''){
	...
	public function restore(){
		if($this->input->post('is_ajax')==2) {
			...
			$id = $this->uri->segment(4);
			$Return['csrf_hash'] = $this->security->get_csrf_hash();
			$result = $this->Designation_model->restore($id);
			if(isset($id))
