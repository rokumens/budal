...
class Files extends MY_Controller
{
  /*Function to set JSON output*/
	...
	public function add_files() {
		if($this->input->post('type')=='file_info') {
			...
			if($this->input->post('department_id') === ''){
				...
				$Return['error'] = 'Please set a name for this file.';
			}elseif($_FILES['xin_file']['size'] == 0 && null ==$this->input->post('remove_profile_picture')) {
				$Return['error'] = $this->lang->line('xin_error_select_file');
			}elseif($_FILES['xin_file']['size'] > $ifilesize) {
	...
	public function files_list() {
		...
		else $file = $this->Files_model->department_files($id);
		$data = array();
    foreach($file->result() as $r) {
			...
			$download = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_download').'"><a href="'.site_url().'admin/download?type=files&filename='.$r->file_name.'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-download"></span></button></a></span>';
			else $functions ='';
			$data[] = array(
				#$download.
				'<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".payroll_template_modal" data-file_id="'. $r->file_id . '" data-field_type="file_manager"><span class="fa fa-pencil"></span></button></span><span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger btn-sm m-b-0-0 waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->file_id . '" data-token_type="document"><span class="fa fa-trash"></span></button></span>',
				$r->name,
				$department_name,
				$fsize,
				// $r->file_extension,
	...
	public function setting_info() {
		if($this->input->post('type')=='setting_info') {
	    	...
	    	$Return['error'] = $this->lang->line('xin_error_max_file_size_required');
			 elseif($this->input->post('allowed_extensions')==='')
	...
	public function delete() {
		if($this->input->post('is_ajax')=='2') {
			...
			if(isset($id)){
				...
				$directory = getcwd()."/uploads/files/";
				$currFile = $this->Files_model->read_file_information($id);
				$filename = $currFile[0]->file_name; //get the filename
				if($filename)
					unlink($directory.$filename); //delete it
				// then finally delete the record.
				$this->Files_model->delete_record($id);
			}else{$Return['error'] = $this->lang->line('xin_error_msg');}
			$this->output($Return);
		}
	}
