...
class Project extends MY_Controller {
  ...
  public function invoices(){
		...
		if($system[0]->module_projects_tasks!='true')
			redirect('admin/dashboard');
		$data['title'] = $this->Xin_model->site_title();
		$data['all_employees'] = $this->Employee_model->all_employees();
		$data['all_companies'] = $this->Xin_model->get_companies();
		$data['all_clients'] = $this->Clients_model->get_all_clients();
		$data['breadcrumbs'] = $this->lang->line('xin_projects');
		$data['path_url'] = 'project';
  ...
  public function project_list() {
			...
			if(in_array('386',$role_resources_ids)){
				 ...
				 foreach($aim as $dIds) {
					 if($session['user_id'] == $dIds) {
						...
						// user full name
						if(!is_null($user))
							$full_name = $user[0]->employee_id.' '.$user[0]->username;
						 else $full_name = '--';
						// get date
						...
						// luffy 2 January 2020 12:23 pm
						$projectTitle = '<div class="text-semibold"><a href="'.site_url().'admin/project/detail/'.$r->project_id . '">'.$r->title.'</a></div>';
						$projectSummary = '<div class="text-muted">'.$r->summary.'</div>';
						if(in_array('316',$role_resources_ids))  // update
						...
						if(in_array('317',$role_resources_ids))  // del
				...
				// luffy
				// $result2 = $this->Clients_model->read_client_info($r->client_id);
				// if(!is_null($result2)) {
				// 	$client_name = $result2[0]->name;
				// } else {
				// 	$client_name = '--';
				...
				if(in_array('318',$role_resources_ids)) { //view
					...
					$projectTitle = '<div class="text-semibold">'.$r->title.'</div>';
					$projectSummary='<div class="text-muted">'.$r->summary.'</div>';
				}
				$combhr = $edit.$delete;
				$data[] = array(
    ...
    foreach($bug->result() as $r) {
			...
			if($ses_user[0]->user_role_id==1)
				$link = '<a class="c-user text-black" href="'.site_url().'admin/employees/detail/'.$r->user_id.'"><span class="underline">'.$employee_name.' ('.$designation_name.')</span></a>';
			else $link = '<span class="underline">'.$employee_name.' ('.$designation_name.')</span>';
			if($r->attachment_file!='' && $r->attachment_file!='no_file'){
