...
class Expense extends MY_Controller {
  ...
  public function expense_list() {
		...
		else $expense = $this->Expense_model->get_expenses();
		$data = array();
    foreach($expense->result() as $r) {
			...
			elseif($r->status==1): $status = $this->lang->line('xin_approved');
				...
				if(in_array('313',$role_resources_ids)) { //view
					$view = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-expense_id="'. $r->expense_id . '"><span class="fa fa-eye"></span></button></span>';
				} else {
					$view = '';
				}
				if(in_array('314',$role_resources_ids)) { //download
					if($r->billcopy_file!='' && $r->billcopy_file!='no file') {
						$download = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_download').'"><a href="download?type=expense&filename='.$r->billcopy_file.'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" title="'.$this->lang->line('xin_download').'"><span class="fa fa-download"></span></button></a></span>';
					} else {
						$download = '';
					}
				} else {
 ...
 public function add_expense() {
		if($this->input->post('add_type')=='expense') {
			...
			}elseif($this->input->post('purchase_date')==='') {
