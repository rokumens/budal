...
class Settings extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function modules()
     {
		...
		$data = array(
			...
			'module_projects_tasks' => $setting[0]->module_projects_tasks,
			'module_chat_box' => $setting[0]->module_chat_box,
			'module_appraisal' => $setting[0]->module_appraisal,
			'module_offday' => $setting[0]->module_offday,
			'module_immigrations' => $setting[0]->module_immigrations
			);
		$role_resources_ids = $this->Xin_model->user_role_resource();
	...
	public function constants(){
		...
		$data['allActiveEmployees'] = $this->Employees_model->employeeApprover()->result();
	...
	public function company_info() {
		...
		}elseif($this->input->post('website')==='') {
			$Return['error'] = $this->lang->line('xin_error_website_field');
		}elseif($this->input->post('contact_person')==='') {
	...
	public function modules_info() {
		...
		'module_performance' => $this->input->get('mperformance'),
		'module_files' => $this->input->get('mfiles'),
		'module_awards' => $this->input->get('mawards'),
		'module_training' => $this->input->get('mtraining'),
		'module_inquiry' => $this->input->get('minquires'),
		'module_language' => $this->input->get('mlanguage'),
		'module_orgchart' => $this->input->get('morgchart'),
		'module_accounting' => $this->input->get('maccounting'),
		'module_events' => $this->input->get('mevents'),
		'module_goal_tracking' => $this->input->get('goal_tracking'),
		'module_assets' => $this->input->get('asset'),
		'module_projects_tasks' => $this->input->get('project'),
		'module_appraisal' => $this->input->get('appraisal'),	// luffy
		'module_offday' => $this->input->get('offday'),	// luffy
		'module_immigrations' => $this->input->get('immigrations')	// luffy
		);
	...
	public function approval_list(){
		...
		$start = intval($this->input->get("start"));
		$length = intval($this->input->get("length"));
		$arrApprover = $this->Xin_model->get_approver_list();
		$data = array();
    foreach($arrApprover->result() as $r) {
			$employee = $this->Employees_model->getNamebyUserId($r->approver);
			$department = $this->Department_model->read_department_information($employee->department_id);
			$designation = $this->Designation_model->read_designation_information($employee->designation_id);
			if(!is_null($employee)){
				$employeeNIKid = $employee->employee_id;
				$employeeUsername = $employee->username;
				$employeeFirstName = $employee->first_name;
				$empoyeeLastName = $employee->last_name;
				$employeeDesignation = $designation[0]->designation_name;
				$employeeDepartment = $department[0]->department_name;
			}else{
				$employeeUsername = '-';
			}
			$data[] = array('
			<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'">
				<button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->approver_id . '" data-token_type="approver">
					<span class="fa fa-trash"></span>
				</button>
			</span>',
				$employeeNIKid.' - '.$employeeUsername,
				$employeeDesignation,
				$employeeDepartment
			);
    }
	  $output = array(
		   "draw" => $draw,
			 "recordsTotal" => $arrApprover->num_rows(),
			 "recordsFiltered" => $arrApprover->num_rows(),
			 "data" => $data
		);
	  echo json_encode($output);
	  exit();
	...
	public function contract_type_info() {
		...
		}elseif($this->input->post('contract_type')==='') {
	...
	public function document_type_info() {
		...
		if($this->input->post('company')==='') {
        	$Return['error'] = $this->lang->line('error_company_field');
		}elseif($this->input->post('document_type')==='') {
	...
	public function award_type_info() {
		...
		if($this->input->post('company')==='') {
        	$Return['error'] = $this->lang->line('error_company_field');
		}elseif($this->input->post('award_type')==='') {
	...
	public function leave_type_info() {
		...
		if($this->input->post('company')==='') {
        	$Return['error'] = $this->lang->line('error_company_field');
		}elseif($this->input->post('leave_type')==='') {
        	$Return['error'] = $this->lang->line('xin_error_leave_type_field');
		}elseif($this->input->post('days_per_year')==='') {
	...
	public function warning_type_info() {
		if($this->input->post('type')=='warning_type_info') {
			...
			if($this->input->post('company')==='') {
      	$Return['error'] = $this->lang->line('error_company_field');
			}elseif($this->input->post('warning_type')==='') {
	...
	// luffy 29 Dec 2019 08:19 pm
	public function add_approver() {
		if($this->input->post('type')=='add_approver') {
			$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
			$Return['csrf_hash'] = $this->security->get_csrf_hash();
			$paramApprover=$this->input->post('approval');
			$existingApprover=$this->Employees_model->getNamebyUserId($paramApprover);
			if($paramApprover==='')
      	$Return['error'] = 'Please choose approver';
			elseif($paramApprover===$existingApprover)
      	$Return['error'] = 'This one you pick is already an approver.';
			if($Return['error']!='')
	    	$this->output($Return);
			$data = array(
				'approver' => $this->input->post('approval'),
			);
			$result = $this->Xin_model->add_approver($data);
			if ($result == TRUE)
				$Return['result'] = "Approver added.";
	...
	public function termination_type_info() {
		if($this->input->post('type')=='termination_type_info') {
			...
			if($this->input->post('company')==='') {
	    	$Return['error'] = $this->lang->line('error_company_field');
			}elseif($this->input->post('termination_type')==='') {
	...
	public function expense_type_info() {
		...
		if($this->input->post('company')==='') {
        	$Return['error'] = $this->lang->line('error_company_field');
		}elseif($this->input->post('expense_type')==='') {
	...
	public function job_type_info() {
		...
		if($this->input->post('company')==='') {
        	$Return['error'] = $this->lang->line('error_company_field');
		}elseif($this->input->post('job_type')==='') {
	...
	public function exit_type_info() {
		...
		if($this->input->post('company')==='') {
        	$Return['error'] = $this->lang->line('error_company_field');
		}elseif($this->input->post('exit_type')==='') {
	...
	public function travel_arr_type_info() {
		...
		if($this->input->post('company')==='') {
        	$Return['error'] = $this->lang->line('error_company_field');
		}elseif($this->input->post('travel_arr_type')==='') {
	...
	public function approval_status() {
		...
		if($this->input->post('type')=='approval_status') {
		/* Define return | here result is used to return user data and error for error message */
		$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
		$Return['csrf_hash'] = $this->security->get_csrf_hash();
		...
		/* Server side PHP input validation */
		if($this->input->post('company_type')==='') {
        	$Return['error'] = 'Approval status field is required.';
		}
		...
		$data = array(
			'name' => $this->input->post('approval_status'),
			'created_at' => date('d-m-Y h:i:s')
		);

		$result = $this->Xin_model->add_approval_status($data);
		if ($result == TRUE) {
			// $Return['result'] = $this->lang->line('xin_company_type_added');
			$Return['result'] = 'Approval status added.';
		} else {
			$Return['error'] = $this->lang->line('xin_error_msg');
		}
		$this->output($Return);
	...
	# luffy 31 December 2019  12:15 pm
	// delete approver in Warning Type seciton
	public function delete_approver() {
		if($this->input->post('type')=='delete_record') {
			$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
			$id = $this->uri->segment(4);
			$Return['csrf_hash'] = $this->security->get_csrf_hash();
			$result = $this->Xin_model->delete_approver_record($id);
			if(isset($id))
				$Return['result'] = 'Approver deleted.';
			else $Return['error'] = $this->lang->line('xin_error_msg');
			$this->output($Return);
		}
	}
