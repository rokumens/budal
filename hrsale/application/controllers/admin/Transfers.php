...
class Transfers extends MY_Controller {
  ...
  public function transfer_list(){
		...
		else $transfer = $this->Transfers_model->get_transfers();
		$data = array();
    foreach($transfer->result() as $r) {
			...
			if(!is_null($employee)){
				...
				$locationFrom = $employee[0]->fingerprint_location;
			} else {
				$employee_name = '--';
				$locationFrom='X';
			}
			...
			if(in_array('211',$role_resources_ids))  // update
			...
			if(in_array('233',$role_resources_ids))  //view
				$view = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-transfer_id="'. $r->transfer_id . '"><span class="fa fa-eye"></span></button></span>';
			else $view = '';
  ...
  public function get_employees() {
	...
	public function add_transfer() {
		if($this->input->post('add_type')=='transfer') {
			...
			}elseif($this->input->post('transfer_date')==='') {
				$Return['error'] = $this->lang->line('xin_transfers_error_date');
			}elseif($this->input->post('transfer_department')==='') {
				$Return['error'] = $this->lang->line('xin_employee_error_department');
			}elseif($this->input->post('transfer_location')==='') {
			...
			if ($result == TRUE) {
				// luffy: add transfer,
				// biar mudah filtering nanti di employee directory
				$dataTransfer = array(
					'transfer_to' => $this->input->post('transfer_location')
				);
				$employeeIdToTransfer=$this->input->post('employee_id');
				$this->Employees_model->update_record($dataTransfer,$employeeIdToTransfer);
