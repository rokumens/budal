...
class Dashboard extends MY_Controller {
	...
	public function employee_department()
	{
		...
		foreach($this->Department_model->all_departments() as $department) {
			...
			//luffy 12 Feb 2020 03:35 pm | show only active employee
	...
	public function employee_designation()
	{
		...
		foreach($this->Designation_model->all_designations() as $designation) {
			//luffy 12 Feb 2020 03:35 pm | show only active employee
			$condition = "designation_id = $designation->designation_id AND is_active=1 AND deleted_at IS NULL and fingerprint_location!=0";
			$this->db->select('*');
			$this->db->from('xin_employees');
			$this->db->where($condition);
			$query = $this->db->get();
