...
class Announcement extends MY_Controller {
    ...
    public function announcement_list(){
	    ...
	    foreach($announcement->result() as $r) {
				...
				if(in_array('255',$role_resources_ids))  //update
	 ...
	 public function deleted() {
			...
			if(empty($session))
				redirect('admin/');
			$data['title'] = 'Deleted '.$this->lang->line('xin_announcements').' | '.$this->Xin_model->site_title();
			$data['breadcrumbs'] = $this->lang->line('xin_announcements');
			$data['path_url'] = 'announcements';
			$data['get_all_companies'] = $this->Company_model->get_all_companies();
			$data['all_office_locations'] = $this->Location_model->all_office_locations();
			...
			$data['userRole']=$userRole;
			if(in_array('11',$role_resources_ids)) {
				$data['subview'] = $this->load->view("admin/announcement/announcement_list_deleted", $data, TRUE);
				$this->load->view('admin/layout/layout_main', $data); //page load
			} else {
				redirect('admin/dashboard');
			}
    }
    public function announcement_list_deleted(){
			$data['title'] = $this->Xin_model->site_title();
			$session = $this->session->userdata('username');
			$this->load->view("admin/announcement/announcement_list_deleted", $data);
			$draw = intval($this->input->get("draw"));
			$start = intval($this->input->get("start"));
			$length = intval($this->input->get("length"));
			$announcement = $this->Announcement_model->get_announcements_deleted();
			$role_resources_ids = $this->Xin_model->user_role_resource();
			$data = array();
	    foreach($announcement->result() as $r) {
				// get date
	 		  ...
	 		  if($user[0]->user_role_id==1)
	 			 if(!empty($session))
	 				 $restore='<span data-toggle="tooltip" data-placement="top" title="Restore"><button type="button" class="btn icon-btn btn-xs btn-success waves-effect waves-light restore" data-toggle="modal" data-target=".restore-modal" data-record-id="'. $r->announcement_id . '"><span class="fa fa-undo"></span></button></span>';
	 		  $action = $restore;
				$data[] = array(
					$action,
					$r->title,
	...
	public function restore(){
		if($this->input->post('is_ajax')==2) {
			...
			$id = $this->uri->segment(4);
			$Return['csrf_hash'] = $this->security->get_csrf_hash();
			$result = $this->Announcement_model->restore($id);
			if(isset($id))
