...
class Timesheet_dev extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function attendance(){
			...
			if(!empty($session)){
				$data['subview'] = $this->load->view("admin/timesheet_dev/attendance_list", $data, TRUE);
	 ...
	 public function attendance_list() {
			 ...
			 if(!empty($session))
				 $this->load->view("admin/timesheet_dev/attendance_list", $data);
			 else redirect('admin/');
		...
		function sendCreatedFingerprintToChannel($approvalId,$clockNotif,$submittedName,$employeeNameForgotFingerprint,$fingerprintDateProposed,$fingerprintClockInProposed,$fingerprintClockOutProposed,$manualFingerprintCreatedAt,$reason){
		  ...
		  $redirectTo=site_url().'admin/timesheet_dev/details/'.$approvalId;
			$array = array(
		    'username' => 'APG Bot',
		 ...
		 public function attendance_approval(){
				...
				if(in_array('1017',$role_resources_ids)) {
					if(!empty($session)){
					$data['subview'] = $this->load->view("admin/timesheet_dev/attendance_approval_list", $data, TRUE);
					$this->load->view('admin/layout/layout_main', $data); //page load
					} else {
						redirect('admin/dashboard/');
					}
		 ...
		 public function attendance_approval_list(){
			 ...
			 $user_info = $this->Xin_model->read_user_info($userId);
			 if(!empty($session))
				 $this->load->view("admin/timesheet_dev/attendance_approval_list", $data);
			 else redirect('admin/');
			 $draw = intval($this->input->get("draw"));
			 $start = intval($this->input->get("start"));
			 $length = intval($this->input->get("length"));
			 ...
			 if(!empty($attendanceApproval)){foreach($attendanceApproval->result() as $r) {
	 				if(($userEmail==='2002@asiapowergames.com')||($userEmail==='9302@asiapowergames.com')||($userEmail==='7200@asiapowergames.com')||($userEmail==='7369@asiapowergames.com')||($userEmail==='8000@asiapowergames.com')||($userEmail==='7380@asiapowergames.com')){
						$viewApproval='<span data-toggle="tooltip" data-placement="top" title="Approval"><a href="'.site_url().'admin/timesheet_dev/details/'.$r->id.'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-arrow-circle-right"></span></button></a></span>';
		...
		public function read_timesheet_approval() {
			...
			$session = $this->session->userdata('username');
			if(!empty($session))
				$this->load->view('admin/timesheet_dev/dialog_timesheet_approval', $data);
			else redirect('admin/');
	 }
	 ...
	 function sendApprovalFingerprintToChannel($approvalIdUrlParam,$clockNotifLabel,$proposedBy,$employeeNameForgotFingerprint,$fingerprintDateProposed,$fingerprintClockProposed,$approvalStatusLabel,$approvedBy){
		 ...
		 $redirectTo=site_url().'admin/timesheet_dev/details/'.$approvalIdUrlParam;
	 ...
	 public function details() {
 		...
 		$result = $this->Fingerprint_model->read_attendance_information($id);
 		if(is_null($result))
 			redirect('admin/timesheet_dev/attendance_approval');
		$attendanceId=$result[0]->id;
		$employeeId=$result[0]->employee_id;
		$employee=$this->Fingerprint_model->getNamebyEmployeeId($employeeId);
		$submittedBy=$this->Fingerprint_model->getNamebyUserId($result[0]->clock_in_added_by);
		...
		$data['title'] = 'Approval detail for '.$fullName;
 		...
 		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(!empty($session)){
		$data['subview'] = $this->load->view("admin/timesheet_dev/approval_details", $data, TRUE);
		$this->load->view('admin/layout/layout_main', $data); //page load
		} else {
			redirect('admin/');
		}
	 ...
	 public function date_wise_attendance(){
		...
		if(in_array('29',$role_resources_ids)) {
			if(!empty($session)){
			$data['subview'] = $this->load->view("admin/timesheet_dev/date_wise", $data, TRUE);
			$this->load->view('admin/layout/layout_main', $data); //page load
			} else {
				redirect('admin/');
			}
	 ...
	 public function update_attendance(){
		...
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(!empty($session)){
			$data['subview'] = $this->load->view("admin/timesheet_dev/update_attendance", $data, TRUE);
			$this->load->view('admin/layout/layout_main', $data); //page load
		} else {
			redirect('admin/');
		}
	...
	public function import() {
		...
		$data['path_url'] = 'import_attendance_dev';
		$data['all_employees'] = $this->Xin_model->all_employees();
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(!empty($session)){
			$data['subview'] = $this->load->view("admin/timesheet_dev/attendance_import", $data, TRUE);
			$this->load->view('admin/layout/layout_main', $data); //page load
		} else {
			redirect('admin/');
		}
	...
	public function ajaxFingerprint(){// Start : 7381-jazz 17jan2020 21:23 - add parameter $type
		...
		if($type == 'range'){
			...
			$arrayType['type'] = $type;
			$arrayType['dateFrom'] = date('Y-m-d', strtotime($dateFrom));
			$arrayType['dateTo'] = date('Y-m-d', strtotime($dateTo));
		}else{
			$arrayType['type'] = $type;
		}
		// response array
		...
		foreach($arrAllLocation as $location){
			...
			if($responsePingDns['response'] != 'Connected'){
				// use local ip
				$connectAddress = $location->local_ip;
				// con`nect response message
				$connectResponse['ping_response_dns'] = $responsePingDns;
				$connectResponse['status']['dns'] = FALSE;
				// check local ip connection
				$responsePingIp = $this->pingIp($location->local_ip, 'ip');
				if($responsePingIp['response'] != 'Connected'){
			...
			$this->fingerprint_location($arrayType, $connectAddress, $location->location_id, $location->location_name, $connectResponse);
			unset($connectResponse);
			$connectResponse = $connectResponseClear;
		}
		exit;
	}
	public function fingerprint_location($arrayType = [], $connectAddress, $location_id, $location_name, $connectResponse)
	{
		$key = 0;
		$timeout = 120;
		$officeName=$location_name;
		// $DNS_Kps='kps.kingspace.net';
		...
		if($fingerprint_kps!=""){
			...
			// filter variable
			if($arrayType['type'] == "today"){// jika param = today ambil data hari ini
				$absenKPS = array_filter($absenKPSx, 'filter_tgl');
			}elseif($arrayType['type'] == "range"){// jika param = range ambil data berdasarkan range tgl
				$absenKPS = array_filter($absenKPSx, function($k) use ($arrayType){
					$timeKps = $k['waktu'];
					if(date('Y-m-d', strtotime($timeKps)) >= date('Y-m-d', strtotime($arrayType['dateFrom'])) && date('Y-m-d', strtotime($timeKps)) <= date('Y-m-d', strtotime($arrayType['dateTo']))){
						return TRUE;
					}else{
						return FALSE;
					}
				});
			}else{ // kosong/null ambil semua data
				$absenKPS = $absenKPSx;
			}
			...
			if(count($absenKPS) > 0){
				...
				if(count($absenKPSz) > 0){
					...
					---------------------------------------------------------------------------------------------------------------------------------------------------
					<br />";
					var_dump('There\'s no data to be saved.<br>');
				}
			}else{
				//luffy
				echo "
					---------------------------------------------------------------------------------------------------------------------------------------------------
					<br />";
				var_dump('There\'s no data should be saved.<br>');
			}
			...
			if($connectResponse['status']['dns'] == FALSE){
				$messageOutput = "
					...
					<strong>Detail Problem : </strong><br>
					<strong>DNS : </strong><br>
					".$connectResponse['ping_response_dns']['message']."<br>
				";
			}else{
				$messageOutput = "
					<br />Imported <span style='color:#1DD15E;font-weight:bold;'>Successfully</span>. <br />Execution time: 
				".round($time2-$time1).' seconds.<br />';
			}
			echo $messageOutput.'<br>';
			return TRUE;exit;
		}else{
			if(filter_var($ip_kps, FILTER_VALIDATE_IP)){
				$messageOutput = "
				---------------------------------------------------------------------------------------------------------------------------------------------------
				...
				".$connectResponse['ping_response_ip']['message']."<br>
				";
				echo $messageOutput;
				$titleNotif = 'Fingerprint Problem '.$officeName;
				$messageNotifDns = $connectResponse['ping_response_dns']['plain_message'];
				$messageNotifIp = $connectResponse['ping_response_ip']['plain_message'];
				$titleNotif2 = "";
				// send slack notif error
				$this->send_notif_slack($arrayType['machine'], $titleNotif, $messageNotifDns, $messageNotifIp);
				// echo "<br />========================================";
				// header("refresh:3; url=".site_url('admin/timesheet_dev/getdatafingerprint'));
				return FALSE;die;
			}else{
				$time1 = microtime(true);
				echo "
	...
	/* fingerprint KPS office 1 */
	public function fingerprintKps_1($type = NULL){// Start : 7381-jazz 17jan2020 21:23 - add parameter $type
		$key = 0;
		$timeout = 120;
		$officeName='KPS Office 1';
		// $DNS_Kps='kps.kingspace.net';
		$DNS_Kps='kps1.kanonhost.com';
	...
	public function fingerprintKps_2($type = NULL){// Start : 7381-jazz 17jan2020 21:23 - add parameter $type
		...
		if($fingerprint_kps!=""){
			...
			//luffy
			echo "<br />Imported <span style='color:#1DD15E;font-weight:bold;'>successfully</span>. <br />Execution time: ";echo round($time2-$time1).' seconds.<br />';
			echo "<br />&raquo; Now timesheet attendance is up-to-date, you can click <a href=".site_url()."admin/timesheet_dev/attendance>here</a> to check it.";
			return TRUE;exit;
		}else{
			echo "<br />$officeName fingerprint:<br /><span style='color:#F1323A;font-weight:normal;'>Ouch!</span> there is a problem on <a href='http://$DNS_Kps' target='_blank'>$officeName</a> DNS right now. <br />You can try again by clicking the import button.<br />";
			// echo "<br />========================================";
	...
	public function office_shift() {
		...
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(!empty($session)){
			$data['subview'] = $this->load->view("admin/timesheet_dev/office_shift", $data, TRUE);
			$this->load->view('admin/layout/layout_main', $data); //page load
		} else {
			redirect('admin/');
		}
	...
	public function holidays() {
		...
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(!empty($session)){
			$data['subview'] = $this->load->view("admin/timesheet_dev/holidays", $data, TRUE);
			$this->load->view('admin/layout/layout_main', $data); //page load
		} else {
			redirect('admin/');
		}
	...
	public function leave() {
		...
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(!empty($session)){
			$data['subview'] = $this->load->view("admin/timesheet_dev/leave", $data, TRUE);
			$this->load->view('admin/layout/layout_main', $data); //page load
		} else {
			redirect('admin/');
		}
	...
	public function leave_details() {
		...
		if(is_null($result))
			redirect('admin/timesheet_dev/leave');
		// get leave types
		...
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(!empty($session)){
			$data['subview'] = $this->load->view("admin/timesheet_dev/leave_details", $data, TRUE);
			$this->load->view('admin/layout/layout_main', $data); //page load
		} else {
			redirect('admin/');
		}
 ...
 public function task_details() {
		...
		if(is_null($result))
			redirect('admin/timesheet_dev/tasks');
		...
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(!empty($session)){
			$data['subview'] = $this->load->view("admin/timesheet_dev/tasks/task_details", $data, TRUE);
			$this->load->view('admin/layout/layout_main', $data); //page load
		} else {
			redirect('admin/');
		}
	 ...
	 public function tasks() {
		...
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(!empty($session)){
			$data['subview'] = $this->load->view("admin/timesheet_dev/tasks/task_list", $data, TRUE);
			$this->load->view('admin/layout/layout_main', $data); //page load
		} else {
			redirect('admin/');
		}
	...
	public function task_users() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){
			$this->load->view("admin/timesheet_dev/tasks/get_task_users", $data);
		} else {
			redirect('admin/');
		}
		$draw = intval($this->input->get("draw"));
  ...
  // Validate and update info in database // update_status
	public function update_task_status() {
	 ...
	 public function task_list() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session))
			$this->load->view("admin/timesheet_dev/leave", $data);
		else redirect('admin/');
		$draw = intval($this->input->get("draw"));
		$start = intval($this->input->get("start"));
		$length = intval($this->input->get("length"));
				...
				$aim = explode(',',$r->assigned_to);
				 foreach($aim as $dIds) {
					...
					if(in_array('322',$role_resources_ids))  //view
						$view = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view_details').'"><a href="'.site_url().'admin/timesheet_dev/task_details/id/'.$r->task_id.'/"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-arrow-circle-right"></span></button></a></span>';
					else $view = '';
    ...
    foreach($task->result() as $r) {
	 ...
	 public function comments_list() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session))
			$this->load->view("admin/timesheet_dev/tasks/task_details", $data);
		else redirect('admin/');
		$draw = intval($this->input->get("draw"));
		$start = intval($this->input->get("start"));
		$length = intval($this->input->get("length"));
  ...
  public function attachment_list() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session))
			$this->load->view("admin/timesheet_dev/tasks/task_list", $data);
		else redirect('admin/');
		$draw = intval($this->input->get("draw"));
		$start = intval($this->input->get("start"));
		$length = intval($this->input->get("length"));
 ...
 public function get_employees() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session))
			$this->load->view("admin/timesheet_dev/get_employees", $data);
		else redirect('admin/');
		$draw = intval($this->input->get("draw"));
		$start = intval($this->input->get("start"));
		$length = intval($this->input->get("length"));
 ...
 public function get_leave_employees() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session))
			$this->load->view("admin/timesheet_dev/get_leave_employees", $data);
		else redirect('admin/');
		$draw = intval($this->input->get("draw"));
		$start = intval($this->input->get("start"));
		$length = intval($this->input->get("length"));
 ...
 public function get_employees_leave() {
 	...
 	public function get_company_project() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session))
			$this->load->view("admin/timesheet_dev/tasks/get_company_project", $data);
		else redirect('admin/');
		$draw = intval($this->input->get("draw"));
		$start = intval($this->input->get("start"));
		$length = intval($this->input->get("length"));
	...
	// get company > employees
	 public function get_company_employees() {
			...
			$session = $this->session->userdata('username');
			if(!empty($session)){
				$this->load->view("admin/timesheet_dev/tasks/get_employees", $data);
			} else {
				redirect('admin/');
			}
	...
	// get company > employees
	 public function get_update_employees() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){
			$this->load->view("admin/timesheet_dev/get_update_employees", $data);
		} else {
			redirect('admin/');
		}
		// Datatables Variables
  ...
  public function date_wise_list() {
		...
		if(!empty($session))
			$this->load->view("admin/timesheet_dev/date_wise", $data);
		else redirect('admin/');
		$draw = intval($this->input->get("draw"));
		$start = intval($this->input->get("start"));
		$length = intval($this->input->get("length"));
	 ...
	 public function update_attendance_list() {
		...
		if(!empty($session))
			$this->load->view("admin/timesheet_dev/update_attendance", $data);
		 else redirect('admin/');
		$draw = intval($this->input->get("draw"));
		$start = intval($this->input->get("start"));
		$length = intval($this->input->get("length"));
	...
	public function office_shift_list() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session))
			$this->load->view("admin/timesheet_dev/office_shift", $data);
		 else redirect('admin/');
		$draw = intval($this->input->get("draw"));
		$start = intval($this->input->get("start"));
		$length = intval($this->input->get("length"));
	...
	public function holidays_list() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session))
			$this->load->view("admin/timesheet_dev/holidays", $data);
		else redirect('admin/');
		// Datatables Variables
		$draw = intval($this->input->get("draw"));
		$start = intval($this->input->get("start"));
	...
	public function leave_list() {
		...
		foreach($leave->result() as $r) {
			...
			if(in_array('288',$role_resources_ids)) { //view detail
				$view = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view_details').'"><a href="'.site_url().'admin/timesheet_dev/leave_details/id/'.$r->leave_id.'/"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-arrow-circle-right"></span></button></a></span>';
			} else {
				$view = '';
			}
			$combhr = $view.$delete;
	...
	public function update_attendance_add() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session))
			$this->load->view('admin/timesheet_dev/dialog_attendance', $data);
	  else redirect('admin/');
	}
	// Validate and add info in database
	...
	public function read_task_record() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){
			$this->load->view('admin/timesheet_dev/tasks/dialog_task', $data);
		} else {
			redirect('admin/');
		}
	}
	...
	public function read_leave_record() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){
			$this->load->view('admin/timesheet_dev/dialog_leave', $data);
		} else {
			redirect('admin/');
		}
	}
	...
	public function read_holiday_record() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){
			$this->load->view('admin/timesheet_dev/dialog_holiday', $data);
		} else {
			redirect('admin/');
		}
	}
	...
	public function read_shift_record() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session))
			$this->load->view('admin/timesheet_dev/dialog_office_shift', $data);
		else redirect('admin/');
	}
	//read_map_info
	public function read_map_info() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){
			$this->load->view('admin/timesheet_dev/dialog_read_map', $data);
		} else {
			redirect('admin/');
		}
	}
