...
<div class="modal-header">
  ...
  <h4 class="modal-title" id="edit-modal-data"><?=$this->lang->line('xin_edit_announcement');?></h4>
</div>
...
<?=form_open('admin/announcement/update/'.$announcement_id, $attributes, $hidden);?>
  <div class="modal-body">
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              ...
              <input class="form-control d_date" name="start_date_modal" readonly="true" type="text" value="<?=$start_date;?>">
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group">
              <label for="end_date"><?=$this->lang->line('xin_end_date');?></label>
              <input class="form-control d_date" name="end_date_modal" readonly="true" type="text" value="<?=$end_date;?>">
            </div>
          </div>
        </div>
        <div class="row">
          <div class="col-md-12">
            <div class="form-group">
              ...
              <select class="form-control" name="company_id" id="ajx_company" data-plugin="select_hrm" data-placeholder="<?=$this->lang->line('module_company_title');?>">
                <option value=""></option>
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="form-group" id="department_ajx">
              ...
              <label for="designation" class="control-label"><?=$this->lang->line('xin_department');?></label>
              <select class="form-control" name="department_id" data-plugin="select_hrm" data-placeholder="<?=$this->lang->line('xin_department');?>">
                ...
                <option value="<?=$department->department_id?>" <?php if($department->department_id==$department_id):?> selected <?php endif;?>><?=$department->department_name?></option>
                <?php } ?>
              </select>
            </div>
          </div>
    ...
    <div class="form-group">
      ...
      <textarea class="form-control" placeholder="<?=$this->lang->line('xin_summary');?>" name="summary" cols="30" rows="3" id="summary"><?=$summary;?></textarea>
    </div>
  </div>
			...
			$.ajax({
				...
				success: function (JSON) {
					if (JSON.error != '') {
						...
						"bDestroy": true,
						"ajax": {
							url : "<?=site_url("admin/announcement/announcement_list") ?>",
							type : 'GET'
						},
...
<div class="modal-header">
  <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">×</span> </button>
  <h4 class="modal-title" id="edit-modal-data"><?=$this->lang->line('xin_view_announcement');?></h4>
</div>
<form class="m-b-1">
  <div class="modal-body">
   <div class="table-responsive" data-pattern="priority-columns">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td style="display: table-cell;"><?php foreach($get_all_companies as $company) {?><?php if($company->company_id==$company_id):?> <?=$company->name?> <?php endif;?><?php } ?></td>
      </tr>
      <tr>
        <th><?=$this->lang->line('xin_department');?></th>
        <td style="display: table-cell;"><?php foreach($all_departments as $department) {?><?php if($department->department_id==$department_id):?> <?=$department->department_name?> <?php endif;?><?php } ?></td>
      </tr>
      <tr>
        <th><?=$this->lang->line('xin_summary');?></th>
        <td style="display: table-cell;"><?=html_entity_decode($summary);?></td>
      </tr>
      <tr>
