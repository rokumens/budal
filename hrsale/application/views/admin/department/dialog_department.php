...
<div class="modal-header"> <?=form_button(array('aria-label' => 'Close', 'data-dismiss' => 'modal', 'type' => 'button', 'class' => 'close', 'content' => '<span aria-hidden="true">×</span>')); ?>
  <h4 class="modal-title" id="edit-modal-data"><?=$this->lang->line('xin_department_edit');?></h4>
</div>
...
<?=form_open('admin/department/update/'.$department_id, $attributes, $hidden);?>
<div class="modal-body">
  <div class="form-group">
    <label for="department-name" class="form-control-label"><?=$this->lang->line('xin_name');?>:</label>
  ...
  <div class="form-group" id="employee_ajx">
    <label for="name"><?=$this->lang->line('xin_department_head');?></label>
    <select name="employee_id" id="select2-demo-6" class="form-control" data-plugin="select_hrm" data-placeholder="<?=$this->lang->line('xin_department_head');?>">
      <option value=""></option>
      <?php $result = $this->Employees_model->get_attendance_employees();?>
				...
				success: function (JSON) {
					if (JSON.error != '') {
						...
						var xin_table = $('#xin_table').dataTable({
							"bDestroy": true,
							"ajax": {
								url : "<?=htmlspecialchars(site_url("admin/department/department_list")); ?>",
								type : 'GET'
							},
							dom: 'lBfrtip',
