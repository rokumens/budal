...
{ 
  ...
  public function index()
  {
    ...
    $pageName = 'interested';
    return view('numbers.followup.interested', compact('nextAction', 'constant', 'roles', 'catGames', 'catWeb', 'pageName'));
  }
  ...
  function getdata(Request $request)
  {
    ...
    if(($user->can('list-interested-all') && $user->can('list-interested-own')) || $user->can('list-interested-all')){
      if(!empty($startDateParam) && !empty($endDateParam)){
        ...
        $data = Interested::
                      leftJoin('master_numbers', 'interested.master_numbers_id', '=', 'master_numbers.id')
                        ->leftJoin('category_web', 'interested.category_web', '=', 'category_web.id')
                        ->leftJoin('category_game', 'interested.category_game', '=', 'category_game.id')
                        ->select(
                          'interested.id', 'interested.created_at', 'interested.master_numbers_id',
                      ...
                      ->whereBetween('interested.created_at', [$startDate, $endDate]) //agar lebih akurat, pakai contacted_date
                      ->orderBy('interested.id', 'desc');
      }else{
        $data = Interested::
                        leftJoin('master_numbers', 'interested.master_numbers_id', '=', 'master_numbers.id')
                        ->leftJoin('category_web', 'interested.category_web', '=', 'category_web.id')
  ...
  public function assignto(Request $request)
  {
    ...
    $checkNumbers = Interested::whereIn('id',explode(",",$idChecked))->get();
    $masterNumbersId = [];
  ...
  public function update(Request $request, $id)
  {
    ...
    $Interested = Interested::FindOrFail($id);
    // if connect response not active
    if($input['is_registered'] != 1){
      // just update master table if is_registered not equal 1
      $dataMaster = [
        'next_action_interested' => $input['next_action'],
        'is_registered' => $input['is_registered'],
        'note_interested' => $input['note_interested'],
      ];
      ...
      Registered::insert($dataRegistered);
      // delete from interested table
      $Interested->forceDelete($id);
    }
    $result = 0;
    if ($result == 0) {
  ...
  public function search(Request $request)
  {
    ...
    if ($user->isCs()) {
      $results = Interested::
                          ...
                          ->orderBy('interested.id', 'desc')->get();
    }else{
      $results = Interested::
                          leftJoin('master_numbers', 'interested.master_numbers_id', '=', 'master_numbers.id')
                          ->leftJoin('category_web', 'interested.category_web', '=', 'category_web.id')
  ...
  public function getInterestedAjax(Request $request)
  {
    $post = $request->all();
    $data = MasterNumbers::findOrFail($post['master_numbers_id']);
