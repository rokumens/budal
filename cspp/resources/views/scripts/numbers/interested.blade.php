<script type="text/javascript">
  $(document).ready(function () {
    ...
    var dataTableAdminLeader = $('#clients_ajax_table_server_side').DataTable({
        ...
        "ajax":{
            url: "{{ route('ajaxdata.getdataInterested') }}",
            data: function(d) {
      ...
      $(document).on('click', '.editModalBtn', function(){
        ...
        $.ajax({
          ...
          success:function(data){
            if(data.is_registered == 0){
              $('#note_interested').prop("disabled", false);
            }else if(data.is_registered == 1){
              $('#note_interested').prop("disabled", false);
            }else{
              $('#note_interested').prop("disabled", true);
            }
            $('#is_registered').val(data.is_registered);
            $('#note_interested').val(data.note_interested);
            $('#next_action').val(data.next_action_interested);
            // disabled note registered when is registerd not selected
            $('#is_registered').change(function(){
              const is_registered = $(this).find(":selected").val();
              if(is_registered == ""){
                $('#note_interested').prop("disabled", true);
              }else{
                $('#note_interested').prop("disabled", false);
        ...
        $('form#classForm').attr('action', '{{URL::to("interested-update")}}/'+id);
        // button save handler
        $(document).on('click', '.btn-save', function(event){
          if($('#is_registered').val() == ""){
            event.preventDefault();
            alert('Please fill the is interested');
          }
          if($('#next_action').val() == ''){
            event.preventDefault();
            alert('Please fill next action');
          }
        });
      });
    ...
    $(document).on('click', '.viewModalBtn', function(){
      ...
      $.ajax({
        ...
        success: function(data){
          ...
          $('.modal-title').html('Interested Detail');
