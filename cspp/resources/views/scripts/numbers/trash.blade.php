<script type="text/javascript">
    $(document).ready(function () {
      ...
      var dataTableAdminLeader = $('#clients_ajax_table_server_side').DataTable({
          ...
          "ajax":{
              url: "{{ route('ajaxdata.getdataTrash') }}",
              data: function(d) {
      ...
      $(document).on('click', '.viewModalBtn', function(){
        ...
        $.ajax({
          ...
          success: function(data){
            ...
            $('#deleted_by').html(data.deleted_by_nik + ' - ' + data.deleted_by_name);
            $('#deleted_at').html(moment(data.deleted_at).locale('id').format('LLLL'));
            $('#checked_by').html(data.users_leader_nik + ' - ' + data.users_leader_name);
            $('#checked_date').html(moment(data.check_1_2).locale('id').format('LLLL'));
            $('#connect_response_by_leader').html(data.connect_response_leader_name);
            $('#checked_note').html(data.check_1_4);
            ...
            if (jsonData.length != 0) {
              $.each(jsonData, function(index, val) {
                resultsContainer.append('<tr>' +
                  ...
                  '<td><a href="#" class="btn btn-sm btn-success viewModalBtn" data-toggle="modal" data-id="' + val.id  +'" data-master_numbers_id="' + val.master_numbers_id  +'">View</a></td>' +
                '</tr>');
              });
            } else {
