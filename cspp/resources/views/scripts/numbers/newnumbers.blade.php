<script type="text/javascript">
  $(document).ready(function () {
    ...
    var dataTableAdminLeader = $('#clients_ajax_table_server_side').DataTable({
        ...
        "ajax":{
            url: "{{ route('ajaxdata.getdataNewnumbers') }}",
            data: function(d) {
        ...
        "columns":[
          @can('assign-cs')
          {
            ...
            render: function ( data, type, full, meta ) {
                var itemsId = full.id;
                return '<div class="custom-control custom-checkbox text-center" style="position:relative; left:-8px;"><input type="checkbox" class="custom-control-input checkbox" id="chk' + itemsId + '" data-id="' + itemsId + '"><label class="custom-control-label" for="chk' + itemsId + '"></label></div>';
            },
          },
          @endcan
          { "data": "phone" },
    ...
    $(document).on('click', '.viewModalBtn', function(){
      ...
      $.ajax({
        ...
        type : 'post',
        success: function(data){
          $('.modal-title').html('New Numbers Detail');
