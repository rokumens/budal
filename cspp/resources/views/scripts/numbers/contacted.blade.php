<script type="text/javascript">
  $(document).ready(function () {
    ...
    var dataTableAdminLeader = $('#clients_ajax_table_server_side').DataTable({
        ...
        "ajax":{
            url: "{{ route('ajaxdata.getdataContacted') }}",
            data: function(d) {
    ...
    $(document).on('click', '.editModalBtn', function(){
      ...
      $.ajax({
        ...
        success:function(data){
          ...
          $('textarea#note_contacted').val(data.note_contacted);
        }
      });
      ...
      $('#connectresponse').change(function(){
        ...
        if(connectResponse == 1){
          ...
          $('.hide_note').hide();
          $('.hide_note').removeAttr('required');
          $('#campaignresult').prop("disabled", false);
          $('#campaignresult').val('');
          $('textarea#note_contacted').attr('placeholder', "Tell us your call result with this number...");
        }else if(connectResponse == ''){
          $('.responseDesc').attr('style', 'display: none;');
          $('#campaignresult').prop("disabled", true);
          $('#campaignresult').val("");
          $('.hide_note').hide();
          $('.hide_note').removeAttr('required');
          ...
          $('textarea#note_contacted').attr('placeholder', "Tell us your call result in more detail why this number "+valuesConnectresponse[connectResponse].toLowerCase()+ '...');
          $('.responseDesc').attr('style', 'display: block;');
          $('.responseDesc').html('<strong>Note : </strong>'+msg[connectResponse]);
          $('#campaignresult').prop("disabled", true);
      ...
      $('#campaignresult').change(function(){
        const campaignResult = $(this).find(":selected").val();
        const connectResponse = $('#connectresponse').find(":selected").val();
        if(connectResponse == ''){
          $('#campaignresult').prop("disabled", true);
          $('#campaignresult').val("");
          $('.hide_note').hide();
          $('.hide_note').removeAttr('required');
        }else if(connectResponse == 1 && campaignResult == ''){
          $('.hide_note').hide();
          $('.hide_note').removeAttr('required');
        }else if(connectResponse == 1 && campaignResult == 1){
          $('.hide_note').hide();
          ...
          const optionsCampaignresult = $('#campaignresult option');
          const valuesCampaignresult = $.map(optionsCampaignresult ,function(option) {
              return [option.text];
          });
          $('textarea#note_contacted').attr('placeholder', "Tell us your call result in more detail why this number "+valuesCampaignresult[campaignResult].toLowerCase()+ '...');
          $('.hide_note').show();
        }else{
          $('.hide_note').show();
          $('textarea#note_contacted').attr('placeholder', "Tell us your call result with this number...");
      ...
      $(document).on('click', '.btn-save', function(event){
        ...
        }else if($('#campaignresult').val() == ""){
          event.preventDefault();
          alert('Please fill campaign result');
        }else{
          $('select#campaignresult').removeAttr('disabled');
          $('select#campaignresult').attr('readonly', 'readonly');
          return true;
        }
      });
    });
    ...
    $(document).on('click', '.viewModalBtn', function(){
      ...
      $.ajax({
        ...
        success: function(data){
          ...
          $('.modal-title').html('Contacted Detail');
